...
public cmdKick(id, level, cid)
{
	...
	new authid[32], authid2[32], name2[MAX_NAME_LENGTH], name[MAX_NAME_LENGTH], userid2, reason[64]
	...
	console_print(id, "[OFFICIAL] Client ^"%s^" kicked", name2)
...
public cmdUnban(id, level, cid)
{
	...
	if (contain(arg, ".") != -1)
	{
		...
		console_print(id, "[OFFICIAL] %L", id, "IP_REMOVED", arg)
	} else {
		...
		console_print(id, "[OFFICIAL] %L", id, "AUTHID_REMOVED", arg)
	}
...
public cmdAddBan(id, level, cid)
{
	...
	if (!(get_user_flags(id) & ADMIN_RCON))
	{
		...
		if (isip)
		{
			...
			for (new i = 0; i < g_Size; i++)
			{
				...
				if (equal(IP, arg))
				{
					...
					{
						console_print(id, "[OFFICIAL] %s : %L", IP, id, "CLIENT_IMM", Name);
		...
		else
		{
			...
			for (new i = 0; i < g_Size; i++)
			{
				...
				if (equal(Auth, arg))
				{
					if (Access & ADMIN_IMMUNITY)
					{
						console_print(id, "[OFFICIAL] %s : %L", Auth, id, "CLIENT_IMM", Name);
		...
		if (!canban)
		{
			console_print(id, "[OFFICIAL] You may only ban recently disconnected clients.  Use ^"amx_last^" to view.");
	...
	if (contain(arg, ".") != -1)
	{
		...
		console_print(id, "[OFFICIAL] Ip ^"%s^" added to ban list", arg)
	} else {
		if(!isCommandArgSafe(arg))
		{
			console_print(id, "%l", "CL_NOT_FOUND");
		...
		console_print(id, "[OFFICIAL] Authid ^"%s^" added to ban list", arg)
	}
...
public cmdBan(id, level, cid)
{
	...
	console_print(id, "[OFFICIAL] %L", id, "CLIENT_BANNED", name2)
...
public cmdSlay(id, level, cid)
{
	...
	console_print(id, "[OFFICIAL] %L", id, "CLIENT_SLAYED", name2)
...
public cmdSlap(id, level, cid)
{
	...
	console_print(id, "[OFFICIAL] %L", id, "CLIENT_SLAPED", name2, damage)
...
public cmdMap(id, level, cid)
{
	...
	{
		console_print(id, "[OFFICIAL] %L", id, "MAP_NOT_FOUND")
		return PLUGIN_HANDLED
	}
	...
	if(pluginloaded != -1)
	{
		...
		server_cmd("amx_pausecfg stop AMXX-Server-Relay.amxx")
	}
	...
	if(pluginloadedvip != -1)
	{
		...
		server_cmd("amx_pausecfg stop VIP.amxx")
	}
	set_task(5.0, "chMap", 0, arg, arglen + 1)
...
public cmdCvar(id, level, cid)
{
	...
	{
		console_print(id, "[OFFICIAL] %L", id, "UNKNOWN_CVAR", arg)
		return PLUGIN_HANDLED
	}
	...
	if (onlyRcon(arg) && !(get_user_flags(id) & ADMIN_RCON))
	{
		...
		{
			console_print(id, "[OFFICIAL] %L", id, "CVAR_NO_ACC")
			return PLUGIN_HANDLED
		}
	}
	...
	if (read_argc() < 3)
	{
		...
		console_print(id, "[OFFICIAL] %L", id, "CVAR_IS", arg, arg2)
		return PLUGIN_HANDLED
	}
	...
	for (new i; i<pnum; i++)
	{
		...
		show_activity_id(plr, id, name, "%L", LANG_SERVER, "SET_CVAR_TO", "", arg, cvar_val);
	}
...
public cmdXvar(id, level, cid)
{
	...
	if( xvar == -1 )
	{
		console_print(id, "[OFFICIAL] %L", id, "UNKNOWN_XVAR", arg1)
		return PLUGIN_HANDLED
	}
	...
	if( !arg2[0] ) // get value
	{
		...
		console_print(id, "[OFFICIAL] %L", id, "XVAR_IS", arg1, arg2);
		return PLUGIN_HANDLED;
	}
	...
	{
		console_print(id, "[OFFICIAL] %L", id, "XVAR_NO_ACC");
		return PLUGIN_HANDLED;
	}
	...
	console_print(id, "[OFFICIAL] %L", id, "XVAR_CHANGED", arg1, arg2);
...
public cmdCfg(id, level, cid)
{
	...
	if (!file_exists(arg))
	{
		console_print(id, "[OFFICIAL] %L", id, "FILE_NOT_FOUND", arg)
		return PLUGIN_HANDLED
	}
	...
	console_print(id, "[OFFICIAL] Executing file ^"%s^"", arg)
...
public cmdLBack()
{
	...
	console_print(g_pauseCon, "[OFFICIAL] Server %s", paused)
...
public cmdPause(id, level, cid)
{
	...
	if (!slayer)
	{ 
		console_print(id, "[OFFICIAL] %L", id, "UNABLE_PAUSE") 
		return PLUGIN_HANDLED
	}
	...
	console_print(id, "[OFFICIAL] %L", id, g_Paused ? "UNPAUSING" : "PAUSING")
...
public cmdRcon(id, level, cid)
{
	...
	console_print(id, "[OFFICIAL] %L", id, "COM_SENT_SERVER", arg)
	server_cmd("%s", arg)
...
public cmdLeave(id, level, cid)
{
	...
	for (new b = 1; b <= pnum; ++b)
	{
		...
		if (ires != -1)
		{
			console_print(id, "[OFFICIAL] %L", id, "SKIP_MATCH", nick, ltags[ires])
			continue
		}
		
		if (get_user_flags(b) & ADMIN_IMMUNITY)
		{
			console_print(id, "[OFFICIAL] %L", id, "SKIP_IMM", nick)
			continue
		}
		
		console_print(id, "[OFFICIAL] %L", id, "KICK_PL", nick)
	...
	console_print(id, "[OFFICIAL] %L", id, "KICKED_CLIENTS", count)
...
public cmdNick(id, level, cid)
{
	...
	console_print(id, "[OFFICIAL] %L", id, "CHANGED_NICK", name2, arg2)
