...
new BACKPACK_MODEL[] = "models/w_backpack.mdl"
...
public plugin_init() 
{
	...
	new filename[33], name[33], versionx[33], authorx[33], status[33]
	get_plugin(-1, filename, charsmax(filename), name, charsmax(name), versionx, charsmax(versionx), authorx, charsmax(authorx), status, charsmax(status));
	...
	if(!find_ent_by_class(-1 , "func_bomb_target") && !find_ent_by_class(-1 , "info_bomb_target"))
	{
		...
		pause("dc",filename )
		log_amx("[C4 Timer Count Hud & Sound] Plugin stoped, Not a bomb map !" ) 
	}
	else
	{
		set_task(10.0, "give_c4", 478794, _, _, "a", 6);
	}
	#else
...
public check_bomb()
{
	static terrorist_count, terrorist,terrorists[MAX_PLAYERS];
	get_players(terrorists, terrorist_count, "aceh", "TERRORIST")
	
	new backpack = find_ent_by_model(-1, "weaponbox", BACKPACK_MODEL)
	
	if (backpack && is_valid_ent(backpack))
	{
		if(!task_exists(478794))
		{
			remove_task(478794);
		}
		return 1
	...
	{
		for (new j = 0; j < terrorist_count; j++ )
		{
			terrorist = terrorists[j]
			if (!backpack || !is_valid_ent(backpack))
			{
				user_has_weapon(terrorist, CSW_C4)
				if(!task_exists(478794))
				{
					remove_task(478794);
				}
...
public give_c4()
{
	if (check_bomb() == 0)
	{
		new ent = find_ent_by_class(ent, "weapon_c4")
		...
		if (ent == 0) 
		{
			...
			if ( terrorist_count > 1 )
			{
				new x = random_num(1,terrorist_count)
				terrorist = terrorists[x]
				give_item(terrorist, "weapon_c4")
				cs_set_user_plant(terrorist, 1, 1)
				get_user_name(terrorist, name, charsmax(name))				
				chat_color_admins(0,"!g[!y:::!tOFFICIAL!y:::!g] !yFound no bombs!")
				chat_color_admins(0,"!g[!y:::!tOFFICIAL!y:::!g] !yGiving bomb to! !t%s",name)
			}
