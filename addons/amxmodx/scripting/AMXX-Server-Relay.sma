...
#define SOCK_ERROR_CREATE_SOCKET 1
#define SOCK_ERROR_SERVER_UNKNOWN 2
#define SOCK_ERROR_WHILE_CONNECTING 3
		...
		if (bytesReceived != -1)
		{
			switch(data[0])
			{
				case MessageChat: {
					...
					format(sHostname,charsmax(sHostname),"^4%s",sHostname);
					...
					do {
						...
						client_print_color(0, print_team_default, "^1[ ^4#%s ^1] ^3%s: ^1%s", sHostname, sPlayerName, sMessage);
					} while(result == 1);
				}
				case MessageEvent: {
					new dataIndex = 1;
					...
					client_print_color(0, print_chat, "[ ^4#%s ^1] ^3%s: ^1%s", sHostname, sEventName, sEventData);
				}
				default: {
...
public handleSay(id)
{
	...
	if((get_user_flags(id) & ADMIN_LEVEL) && ((containi( "amx_chat", sCommand ) != -1 ) || (sMessage[0] == '@')))
	{
		return PLUGIN_CONTINUE;
	}
