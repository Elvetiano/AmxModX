			...
			for (new i = start; ; ++i)
			{
				if (i < end)
				{
					if (equal(szValue, g_cvarCmd[i]))
					{
						...
						log_to_file("CVARmenu.log", "[OFFICIAL] ADMIN %s seteaza cvar %s = %s .", name, g_cvarNames[option], g_cvarCmd[i])
						chat_cvar(0, "!g[OFFICIAL] ADMIN !team%s !y: seteaza cvar !g%s !y = !g%s", name, g_cvarNames[option], g_cvarCmd[i])
						break;
					}
				} 
				else 
