...
#define VERSION	"1.0.6 (amx 1.8.4)"
...
public plugin_init()
{
	...
	pcvar = create_cvar("afk_slay_time", "3", FCVAR_NONE, "Time in seconds after which client gets information for how long is AFk and SLAY", .has_min = true, .min_val = 0.0)
	bind_pcvar_float(pcvar, SlayInformTime)
	
	pcvar = create_cvar("afk_slay_time", "10", FCVAR_NONE, "Time in seconds that every clients can be AFK before being slayed^nSet this cvar to 0 to disable this feature", .has_min = true, .min_val = 0.0)
...
public afk_manager_loop(ent)
{
	...
	for ( i = 0; i < player_count; i++ )
	{
		...
		if(SlayInform > 0)
		{
			if (AfkTime[player] > SlayInformTime)
			{
				if ( ColoredMessages )
				{
					set_dhudmessage(0, 50, 255, 0.02, 0.688, 0, 0.0, 0.8, 0.0, 0.0)
				}
				else
				{
					set_dhudmessage(255, 255, 255, 0.02, 0.688, 0, 0.0, 0.8, 0.0, 0.0)
				}	
				show_dhudmessage(player, "[AFK Manager] You are AFK for %d, slay in %d", afk_time, (SlayTime - afk_time))
			}
		}
	}
